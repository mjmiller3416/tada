...
supply runs low/out it gets pushed into MealGenie's shopping list exactly
once per low-run, deduped via `last_pushed_at`:

- mark low/out  -> push to MealGenie if not already pushed this run,
...
def set_status(supply: Supply, status: str) -> bool:
    ...
    MealGenie's shopping list as a result. Caller commits."""
